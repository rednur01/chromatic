...
pub fn main(init: std.process.Init) !void {
    ...
    const purple_oklch: Oklch = try .parse("oklch(70% 0.1 338)");
    ...
    const purple_hex: Hex = .fromRgb(.fromOklch(purple_oklch));
    const purple_hex_str = try purple_hex.stringify();
    std.debug.print("Purple hex: {s}\n", .{purple_hex_str});
}
